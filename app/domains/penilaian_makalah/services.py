...
class PenilaianService:
    def __init__(self, db_repo: EvaluationRepository, minio_repo: MinioRepository, rag_instance, ingest_repo=None):
        self.db_repo = db_repo
        self.minio_repo = minio_repo
        self.rag = rag_instance
        self.ingest_repo = ingest_repo

    def get_available_jabatan(self) -> list[str]:
        """Mengambil daftar nama file di bucket SKJ dan menghapus ekstensinya."""
        files = self.minio_repo.list_files(self.minio_repo.BUCKET_SKJ)
        jabatans = []
        for f in files:
            name = Path(f).stem
            jabatans.append(name)
        return jabatans

    async def ingest_skj_documents(self) -> dict:
        files = self.minio_repo.list_files(self.minio_repo.BUCKET_SKJ)
        details = []
        total_success = 0
        total_failed = 0
        ...
        for f in files:
            if self.ingest_repo and self.ingest_repo.is_ingested(f):
                already_ingested += 1
                continue

            try:
                data = self.minio_repo.download_file(self.minio_repo.BUCKET_SKJ, f)
                if not data:
                    raise Exception("Gagal download file dari MinIO")
                
                text = DocumentExtractor.extract_from_bytes(data, f)
                if not text.strip():
                    raise Exception("Teks kosong setelah ekstraksi")
                
                if self.rag:
                    await self.rag.ainsert(text)
                
                if self.ingest_repo:
                    self.ingest_repo.log_ingestion(f, "success")
                total_success += 1
                details.append({"filename": f, "status": "success"})
            except Exception as e:
                if self.ingest_repo:
                    self.ingest_repo.log_ingestion(f, "failed", str(e))
                total_failed += 1
                details.append({"filename": f, "status": "failed", "error": str(e)})
    ...
    def upload_document(self, bucket: str, filename: str, content: bytes) -> bool:
        """Mengunggah file dokumen langsung ke MinIO"""
        return self.minio_repo.upload_file(bucket, filename, content)
